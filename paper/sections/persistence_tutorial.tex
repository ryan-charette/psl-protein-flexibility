...
\end{equation}
The new-cell variables $y$ can cancel the component of $Ax$ lying in the column space of $B$. They cannot cancel its orthogonal component. The minimum is therefore $\|(I-BB^\dagger)Ax\|^2$. This is the quadratic form of the persistent upper term. The same matrix also arises by projecting onto the admissible cochains described above. The least-squares interpretation is an algebraic explanation, not an extra fitted model in the protein study.

For a small illustration, take $A=(1,0)^T$ and $B=(1,1)^T$. Then $\|Ax+By\|^2=(x+y)^2+y^2$, minimized at $y=-x/2$, with value $x^2/2$. Keeping only the principal old-column term $A^TA$ would instead give $x^2$. Eliminating the new direction changes the operator. This two-column calculation illustrates why a principal submatrix is not generally the persistent operator; it is not asserted to be a protein complex.
