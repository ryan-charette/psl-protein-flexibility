...
BLUE='tab:blue';RED='tab:red';INK='black';GRAY='0.5'
...
def cloud(ax,pts,sheaf=None,radius=None,center=True):
    if sheaf is not None:
        ...
        if triangles:ax.add_collection3d(Poly3DCollection([pts[list(s)] for s in triangles],facecolor=BLUE,alpha=.09,edgecolor='none'))
        if edges:ax.add_collection3d(Line3DCollection([pts[list(e)] for e in edges],colors=GRAY,linewidths=.6,alpha=.55))
    ax.scatter(*pts.T,s=12,c=BLUE,depthshade=False,edgecolors='white',linewidths=.3)
    if center:ax.scatter(*pts[0],s=85,c=RED,edgecolors='white',linewidths=1,depthshade=False)
...
def theory_figures():
    ...
    for i in range(5):
        for j in range(i+1,5):
            if dist[i,j]<=3:ax.plot(points[[i,j],0],points[[i,j],1],color=RED if i==0 else BLUE,lw=2)
    ax.scatter(points[:,0],points[:,1],s=[150]+[95]*4,c=[RED]+[BLUE]*4,edgecolors='white',zorder=3)
...
def interval(ax,y,estimate,color=BLUE,marker='o',label=None):
...
def performance_figure():
    ...
    for y,mode in enumerate(ORDER):
        for protocol,offset,color,marker in [('cluster',-.1,BLUE,'o'),('protein',.1,GRAY,'s')]:
    ...
    ax.legend(handles=[Line2D([],[],color=BLUE,marker='o',label='Sequence-controlled'),Line2D([],[],color=GRAY,marker='s',label='Protein-only')],frameon=False,fontsize=11,loc='lower left')
    ax.set_ylim(8.6,-.5)
    ax.grid(axis='x',alpha=.13)
    ax=axs[0,1];title(ax,'b','Change from graph geometry')
    for y,mode in enumerate(ORDER[1:]):interval(ax,y,paired(pairs,'graph',mode),RED if mode=='graph+center0' else BLUE)
    ...
    for pid in ('1ULR','1X3O'):
        ...
        ax.annotate(pid,(x,y),xytext=(25,-24) if pid=='1ULR' else (-55,22),textcoords='offset points',fontsize=12,arrowprops={'arrowstyle':'-','color':RED,'lw':.8},bbox={'facecolor':'white','edgecolor':'none','alpha':.8,'pad':1})
    ...
    ax.hist(delta,bins=np.linspace(-extent,extent,31),color=BLUE,alpha=.85,edgecolor='white',linewidth=.5)
    ax.axvline(0,color=GRAY,lw=1,ls='--');ax.axvline(delta.mean(),color=RED,lw=1.7)
...
def persistence_figure():
    ...
    for y,(label,r) in enumerate(records):interval(ax,y,[r.mean_pcc,r.pcc_ci95_low,r.pcc_ci95_high],RED if y==0 else BLUE)
    ...
    ax.barh(range(3),times,color=[GRAY,BLUE,RED],height=.48);ax.set_yticks(range(3),names,fontsize=12);ax.invert_yaxis()
...
def hero_figure(pid='1ULR',readme=False):
    ...
    ax.tick_params(labelsize=10.5);ax.legend(frameon=False,fontsize=10,ncol=2,loc='upper center');ax.grid(axis='y',alpha=.13)
    lo,hi=ax.get_ylim();ax.set_ylim(lo,hi+.35*(hi-lo))
...
def main():
    ...
    args=p.parse_args()
    if args.rerender:
        from explain_cases import render_cases
        ids=('1ULR','1X3O')
